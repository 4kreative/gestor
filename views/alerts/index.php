    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          ...
          <div style="font-size:11px;color:var(--txt3);margin-top:2px">
            <?php if (($a['recipient_type']??'phone')==='group'): ?>
              ...
              <?php if (!empty($a['nome_grupo'])): ?>
                <span title="<?= e($a['recipient_phone']) ?>"><?= e($a['nome_grupo']) ?></span>
              <?php else: ?>
                <?= e($a['recipient_phone']) ?>
              <?php endif; ?>
            <?php else: ?>
        ...
        <td style="font-size:11px">
          ...
          <?php elseif (!empty($a['disparo_imediato']) && ($a['type']??'') !== 'erro_conta'): ?>
          ...
          <?php elseif (($a['type']??'') === 'erro_conta'): ?>
            <div style="display:flex;flex-direction:column;gap:2px">
              <span style="color:#e74c3c;font-size:10px;font-weight:600">🔴 Monitorando conta</span>
              <span style="color:var(--txt3);font-size:10px">Dispara ao detectar erro</span>
            </div>
        ...
        <td>
          ...
          <?php if ($a['ultimo_status'] === 'enviado'): ?>
            <div style="display:flex;flex-direction:column;gap:2px;cursor:pointer" onclick="verLogs(<?= $a['id'] ?>, '<?= e(addslashes($a['name'])) ?>')">
              ...
              <?php endif; ?>
              <?php
                // Para erro_conta: mostra resumo do erro detectado
                if (($a['type']??'') === 'erro_conta'):
                  // Tenta pegar do último log enviado (novo formato com erro_msg)
                  $erroTexto = $a['ultimo_erro_enviado'] ?? null;
                  // Se não tiver, tenta do último erro
                  if (empty($erroTexto)) $erroTexto = $a['ultimo_erro'] ?? null;
                  if (!empty($erroTexto)):
                    $erroResumido = mb_substr($erroTexto, 0, 65);
              ?>
              <span style="font-size:10px;color:#e67e22;max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="<?= e($erroTexto) ?>">
                ⚠️ <?= e($erroResumido) ?>
              </span>
              <?php endif; endif; ?>
            </div>
          ...
          <?php elseif (($a['type']??'') === 'erro_conta'): ?>
            <div style="display:flex;flex-direction:column;gap:2px">
              <span style="font-size:11px;color:#e74c3c;font-weight:600">🔴 Monitorando conta</span>
              <?php
                $erroAtual = $a['ultimo_erro_enviado'] ?? $a['ultimo_erro'] ?? null;
                if (!empty($erroAtual)):
              ?>
              <span style="font-size:10px;color:#e67e22;max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="<?= e($erroAtual) ?>">
                ⚠️ <?= e(mb_substr($erroAtual, 0, 65)) ?>
              </span>
              <?php else: ?>
              <span style="font-size:10px;color:var(--txt3)">Dispara ao detectar erro na conta</span>
              <?php endif; ?>
            </div>
          <?php elseif ($proximoEnvio): ?>
            <div style="display:flex;flex-direction:column;gap:2px">
...
<div style="background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius2);width:100%;max-width:900px;margin:auto;display:grid;grid-template-columns:1fr 300px;overflow:hidden">
  <div style="padding:24px;overflow-y:auto;max-height:90vh">
    ...
    <form method="POST" action="<?= APP_URL ?>/alerts/store" id="alertForm">
      ...
      <div class="form-group" id="modoDisparoGrp" style="display:none">
        ...
        <div style="display:flex;gap:0;border:1px solid var(--border2);border-radius:10px;overflow:hidden;width:fit-content">
          <label id="btnInteligente" style="display:flex;align-items:center;gap:8px;padding:10px 20px;cursor:pointer;font-size:13px;font-weight:600;background:var(--bg3);color:var(--txt2);border-right:1px solid var(--border2);transition:all .2s">
            <input type="radio" name="modo_disparo" value="inteligente" style="display:none" onchange="onModoDisparo()">
            🔔 Inteligente
          </label>
          <label id="btnAgendado" style="display:flex;align-items:center;gap:8px;padding:10px 20px;cursor:pointer;font-size:13px;font-weight:600;background:var(--accent);color:#fff;transition:all .2s">
            <input type="radio" name="modo_disparo" value="agendado" checked style="display:none" onchange="onModoDisparo()">
            📅 Agendado
          </label>
        </div>
        <div id="modoInteligenteHint" style="display:none;margin-top:8px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;padding:10px 14px;font-size:12px;color:var(--txt2)">
        ...
        <div id="modoAgendadoHint" style="margin-top:8px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;padding:10px 14px;font-size:12px;color:var(--txt2)">
      ...
      <!-- Threshold (CTR, CPC, Custo/conv, ROAS) -->
      <div class="form-group" id="thresholdGrp" style="display:none">
        <label class="form-label" id="thresholdLbl">Valor limite</label>
        <input type="number" name="threshold" id="thresholdInp" class="form-control" value="" min="0" step="0.01">
        <div class="form-hint" id="thresholdHint"></div>
      </div>

      <!-- Período de análise (só métricas) -->
      <div class="form-group" id="periodoGrp" style="display:none">
        ...
        <select name="period_type" id="periodTypeSelect" class="form-control">
          ...
          <option value="max">Máximo (desde o início)</option>
          <option value="custom">Personalizado</option>
        </select>
        <div id="customPeriodWrap" style="display:none;margin-top:8px;gap:8px;display:none">
          <input type="date" id="customStart" class="form-control" value="" style="flex:1">
          <input type="date" id="customEnd" class="form-control" value="" style="flex:1">
        </div>
        <div class="form-hint">Período usado para calcular a métrica</div>
      </div>
      ...
      <div class="form-group">
        ...
        <select name="whatsapp_id" class="form-control" id="wpInstSel" onchange="onInstanciaChange(this.value)">
          ...
          <?php foreach ($instances as $idx2 => $inst): ?>
          <option value="<?= $inst['id'] ?>" <?= $idx2===0?'selected':'' ?>><?= e($inst['instance_name']) ?><?= $inst['phone_number']?' ('.$inst['phone_number'].')':'' ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      ...
      <div class="form-group" id="horariosGrp">
...
function setType(el,val){
  ...
  // saldoGrp: só para saldo_minimo e erro_conta
  document.getElementById('saldoGrp').style.display=(!isPerf && val!=='erro_conta')?'':'none';
  // threshold e período: só para métricas de performance
  document.getElementById('thresholdGrp').style.display=isPerf?'':'none';
  document.getElementById('periodoGrp').style.display=isPerf?'':'none';
  // Labels do threshold
  var tlbl=document.getElementById('thresholdLbl'), thint=document.getElementById('thresholdHint');
  if(val==='ctr_baixo'){tlbl.textContent='CTR mínimo (%)';thint.textContent='Dispara quando CTR cair abaixo deste valor';}
  else if(val==='cpc_alto'){tlbl.textContent='CPC máximo (R$)';thint.textContent='Dispara quando CPC ultrapassar este valor';}
  else if(val==='custo_conv_alto'){tlbl.textContent='Custo/conv máximo (R$)';thint.textContent='Dispara quando custo por conversa ultrapassar este valor';}
  else if(val==='roas_baixo'){tlbl.textContent='ROAS mínimo (x)';thint.textContent='Dispara quando ROAS cair abaixo deste valor';}
}
// Período personalizado no modal de criação
    ...
    .then(r=>r.json()).then(function(data){
      ...
      data.logs.forEach(function(l){
        var statusBadge = l.status==='enviado'
          ? '<span style="display:inline-flex;align-items:center;gap:4px;background:#e7f5ec;color:#128C7E;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;white-space:nowrap"><i class="fa-solid fa-check"></i> Enviado</span>'
          : '<span style="display:inline-flex;align-items:center;gap:4px;background:#fdecea;color:#c62828;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;white-space:nowrap"><i class="fa-solid fa-xmark"></i> Erro</span>';
